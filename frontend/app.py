...
with st.sidebar:
    ...
    if st.button("Create a new session"):
        ...
        response = requests.post("http://backend-service:8080/query", json=payload) # use localhost instead of backend-service when testing locally
        data = response.json()
...
# API URL for FastAPI
API_URL = "http://backend-service:8080/query" #use localhost instead of backend-service when testing locally
